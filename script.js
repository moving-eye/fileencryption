$(function(){
	...
	$('a.button.process').click(function(){
		...
		var firstNameInput = $(this).parent().find('input[type=name]'),
			a = $('#step4 a.download'),
			firstName = firstNameInput.val();
			firstNameInput.val('');

		var secondNameInput = $(this).parent().find('input[type=text]'),
		a = $('#step4 a.download'),
		...
		var encryptedFirstName = CryptoJS.AES.encrypt(firstName,password)
		var encryptedSecondName = CryptoJS.AES.encrypt(secondName,password)
		alert('Encrypted values of name and Password\n First Name:'+encryptedFirstName+'\n Second Name:'+ encryptedSecondName);
		...
		if(body.hasClass('encrypt')){
			...
			reader.onload = function(e){
				...
				if(!/^data:/.test(decrypted)){
					alert("Invalid encryption key Please try again.");
					return false;
				}
